...
ExceptionHandler(ExceptionType which)
{
    ...
    else if(which == PageFaultException)
    {
	    ...
	    int virtualPageNo = address / PageSize;
	    printf("PageFaultException vpn %d, badAddrss %d,PageSize %d\n",
	    virtualPageNo,address,PageSize);
			// memoryLock->Acquire();
	    // syscallLock->Acquire();
	    // currentThread->space->PageFault(virtualPageNo);
	    // syscallLock->Release();
			// memoryLock->Release();
			int physPage;
    	if (memoryManager->IsAnyPageFree()){
				printf("got free page\n");
    		physPage = memoryManager->AllocPage(currentThread->id, &(machine->pageTable[virtualPageNo]));
    	}
    	else{
				printf("did not get any free page\n");
    		physPage = memoryManager->AllocByForce();
    	}
			// machine->pageTable[virtualPageNo].valid = true;
			// machine->pageTable[virtualPageNo].physicalPage = physPage;
			currentThread->space->LoadIntoFreePage(virtualPageNo, physPage);
			printf("Loaded finally\n");
			// machine->pageTable[virtualPageNo].valid = true;
			// machine->pageTable[virtualPageNo].physicalPage = physPage;
			// printf("Releasing sysLock\n");
			// syscallLock->Release();
			// memoryLock->Release();
    	printf("PageFaultException: pageFault last\n");
			// ExitProcess();
			// return;
    }
